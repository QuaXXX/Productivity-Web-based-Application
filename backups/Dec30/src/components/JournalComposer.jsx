...
export default function JournalComposer({ onAddEntry, playSound }) {
    ...
    return (
        <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-100 transition-colors duration-500">
            {/* Toggle Header */}
            <div className="flex items-center justify-between mb-4">
                <div className="bg-gray-100/80 p-1 rounded-full flex mx-auto">
                    <button
            ...
            {/* Input Area */}
            <div className={`relative rounded-2xl p-3 min-h-[120px] transition-colors ${type === 'dream' ? 'bg-indigo-50/50' : 'bg-gray-50'}`}>
                <textarea
                    value={text}
                    onChange={(e) => setText(e.target.value)}
                    placeholder={type === 'dream' ? "What did you dream about?..." : "What's on your mind today?..."}
                    className="w-full h-full bg-transparent resize-none outline-none text-gray-700 text-base placeholder:text-gray-400 min-h-[100px]"
                />
            ...
            <div className="flex items-center justify-end mt-3 px-1 gap-2">
                ...
                <button
                    ...
                    className={`p-3 rounded-full transition-all flex items-center space-x-2 ${text.trim()
                        ? (type === 'dream' ? 'bg-indigo-600 text-white shadow-md hover:bg-indigo-700' : 'bg-orange-500 text-white shadow-md hover:bg-orange-600')
