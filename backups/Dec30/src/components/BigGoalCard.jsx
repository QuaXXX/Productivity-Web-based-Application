...
export default function BigGoalCard({
    ...
    return (
        <motion.div
            ...
            className={`rounded-xl ${isCompleted ? 'bg-gray-50 opacity-75' : 'bg-white border-b border-gray-100'} transition-all mb-4 overflow-hidden`}
        >
            ...
            >
                <div className="flex justify-between items-start mb-1 gap-3">
                    ...
                    <div className={`px-2 py-1 rounded-full text-[10px] font-bold flex items-center gap-1 shrink-0 ${isCompleted ? 'bg-gray-200 text-gray-400' : daysLeftColor}`}>
                        {daysLeft !== null ? (daysLeft < 0 ? `${Math.abs(daysLeft)}d Overdue` : `${daysLeft}d Left`) : 'No Date'}
                    </div>
                </div>

                {/* Progress Bar (Slim) */}
                <div className="w-full bg-gray-100 rounded-full h-1.5 mt-3 mb-2 overflow-hidden">
                    <motion.div
