...
const GoalProgressPoster = ({ goal, onHide }) => {
    ...
    const safeSubSteps = subSteps || [];

    // --- Calculations ---
    const totalSteps = safeSubSteps.length;
    const completedSteps = safeSubSteps.filter(s => s.isCompleted).length;
    ...
    const safeParse = (str) => {
        try {
            const d = str ? parseISO(str) : new Date();
            return isNaN(d.getTime()) ? new Date() : d;
        } catch { return new Date(); }
    };

    const start = safeParse(startDate);
    const end = safeParse(dueDate);
    const todayDate = new Date();

    try {
        totalDurationDays = differenceInDays(end, start);
        elapsedDays = differenceInDays(todayDate, start);
    ...
    return (
        <motion.div
            ...
            className={`rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-white/10 relative overflow-hidden transition-all ${isCompleted ? 'bg-gray-50 dark:bg-neutral-900/50 grayscale opacity-80' : 'bg-white dark:bg-neutral-900'
                }`}
        >
            ...
            */}
            ...
            <div className="relative pt-6 pb-2 mb-2">
                ...
                <div className="h-3 bg-gray-100 dark:bg-neutral-800 rounded-full w-full relative">
                    ...
                    >
                        <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 dark:bg-neutral-700 text-white dark:text-gray-100 text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm whitespace-nowrap">
                            Today
                        </div>
                    </div>

                    {/* 4. Sub-step Ticks (Optional visual fluff) */}
                    {safeSubSteps.length > 0 && safeSubSteps.map((step, idx) => {
                        // Distribute ticks evenly
                        const tickPos = ((idx + 1) / safeSubSteps.length) * 100;
                        return (
                            <div
                                key={step.id}
                                className="absolute top-1/2 -translate-y-1/2 w-1 h-1 bg-white dark:bg-neutral-600 rounded-full z-10"
                                style={{ left: `${tickPos}%` }}
                            />
                        )
                    })}
                    ...
                    return (
                        <div
                            ...
                            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium border ${step.isCompleted
                                ...
                                : isFinal
                                    ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 shadow-sm font-bold' // Gold for Final
                                    : 'bg-white dark:bg-neutral-800 border-blue-100 dark:border-neutral-700 text-blue-700 dark:text-blue-400 shadow-sm'
                                }`}
                        >
