...
const WeekView = ({ history, viewingDate, onNavigateToToday, onAddFutureTask, selectedDayIndex, setSelectedDayIndex }) => {
    ...
    return (
        <div className="flex flex-col h-full relative animate-in fade-in duration-200">
            ...
            <div className="flex items-center justify-between mb-6 flex-shrink-0">
                ...
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className="p-1 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                        title="Previous Day"
                    >
                        <ChevronLeft size={28} />
                    </button>
                ...
                <div className="w-10 flex justify-end">
                    {isCanAdd && (
                        <button
                            ...
                            className={`p-2 rounded-full transition-colors ${isAdding ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-300' : 'text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-neutral-800'
                                }`}
                            title="Add Task"
                        >
                            <Plus size={24} />
            ...
            {/* Content List */}
            <div className="flex-1 overflow-y-auto pr-2 -mr-2 pb-16 space-y-3">

                {/* Inline Add Task Form */}
                {isCanAdd && isAdding && (
                    <form onSubmit={handleAddItem} className="bg-[var(--color-surface)] p-3 rounded-xl border border-[var(--color-primary-light)] shadow-[var(--shadow-card)] mb-4">
                        <input
                            autoFocus
                            type="text"
                            ...
                            className="w-full px-3 py-2 border-b border-[var(--color-border-default)] bg-transparent focus:border-[var(--color-primary)] outline-none text-[var(--color-text-primary)] mb-2 placeholder:text-[var(--color-text-tertiary)]"
                        />
